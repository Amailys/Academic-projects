...
def generate(rnn, emb, decoder, eos, start="", maxlen=200):
    ...
    #  TODO:  Implémentez la génération à partir du RNN, 
    # et d'une fonction decoder qui renvoie les logits 
    # (logarithme de probabilité à une constante près, i.e. ce qui vient avant le softmax)
    # des différentes sorties possibles

    n=0
    inputs=[]
    input=emb(string2code(start))
    while (input[-1]!= eos) and n<=maxlen:
        generation=rnn(input)
        logit=decoder(generation)
